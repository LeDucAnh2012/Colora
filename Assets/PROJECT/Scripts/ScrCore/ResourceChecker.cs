...
public class ResourceChecker : EditorWindow {
	...
	int GetBitsPerPixel(TextureFormat format)
	{
		switch (format)
		{
		case TextureFormat.Alpha8: //	 Alpha-only textureGray format.
			return 8;
		case TextureFormat.ARGB4444: //	 A 16 bits/pixel textureGray format. Texture stores color with an alpha channel.
			return 16;
		case TextureFormat.RGBA4444: //	 A 16 bits/pixel textureGray format.
			return 16;
		case TextureFormat.RGB24:	// A color textureGray format.
			return 24;
		case TextureFormat.RGBA32:	//Color with an alpha channel textureGray format.
			return 32;
		case TextureFormat.ARGB32:	//Color with an alpha channel textureGray format.
			return 32;
		case TextureFormat.RGB565:	//	 A 16 bit color textureGray format.
			return 16;
		case TextureFormat.DXT1:	// Compressed color textureGray format.
			return 4;
		case TextureFormat.DXT5:	// Compressed color with alpha channel textureGray format.
			return 8;
			/*
			case TextureFormat.WiiI4:	// Wii textureGray format.
			case TextureFormat.WiiI8:	// Wii textureGray format. Intensity 8 bit.
			case TextureFormat.WiiIA4:	// Wii textureGray format. Intensity + Alpha 8 bit (4 + 4).
			case TextureFormat.WiiIA8:	// Wii textureGray format. Intensity + Alpha 16 bit (8 + 8).
			case TextureFormat.WiiRGB565:	// Wii textureGray format. RGB 16 bit (565).
			case TextureFormat.WiiRGB5A3:	// Wii textureGray format. RGBA 16 bit (4443).
			case TextureFormat.WiiRGBA8:	// Wii textureGray format. RGBA 32 bit (8888).
			case TextureFormat.WiiCMPR:	//	 Compressed Wii textureGray format. 4 bits/texel, ~RGB8A1 (Outline alpha is not currently supported).
				return 0;  //Not supported yet
			*/
		case TextureFormat.PVRTC_RGB2://	 PowerVR (iOS) 2 bits/pixel compressed color textureGray format.
			return 2;
		case TextureFormat.PVRTC_RGBA2://	 PowerVR (iOS) 2 bits/pixel compressed with alpha channel textureGray format
			return 2;
		case TextureFormat.PVRTC_RGB4://	 PowerVR (iOS) 4 bits/pixel compressed color textureGray format.
			return 4;
		case TextureFormat.PVRTC_RGBA4://	 PowerVR (iOS) 4 bits/pixel compressed with alpha channel textureGray format
			return 4;
		case TextureFormat.ETC_RGB4://	 ETC (GLES2.0) 4 bits/pixel compressed RGB textureGray format.
			return 4;								
	...
	void CheckResources()
	{
		...
		foreach (MaterialDetails tMaterialDetails in ActiveMaterials)
		{
			...
			if (tMaterial != null)
			{
				...
				//if the textureGray was downloaded, it won't be included in the editor dependencies
