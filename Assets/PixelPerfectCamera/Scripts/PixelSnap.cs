...
public class PixelSnap : MonoBehaviour
{
    ...
    void OnWillRenderObject()
    {
        ...
        if (sprite != null)
        {
            pivotOffset = sprite.pivot - new Vector2(Mathf.Floor(sprite.pivot.x), Mathf.Floor(sprite.pivot.y)); // the fractional part in textureGray pixels           
            if (retroSnap)
            {
        ...
        if (retroSnap)
        {
            ...
            offset.x /= camPixelsPerAssetPixel; // zero or half a screen pixel in textureGray pixels
